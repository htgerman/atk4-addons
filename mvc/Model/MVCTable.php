...
abstract class Model_MVCTable extends Model {
	...
	*/
	...
	function setOrder($instance=null,$field,$desc=false,$sticky=false){
		...
		// ref_no is string field, while we must sort by numbers
		// also, 'Recurring-' prefix should not be considered
	...
	protected function insertRecord($data=array()) {
		...
		try{
			...
			// TODO: do we need this? we set master id in init...
			if (isset($this->fields['system_id'])&&!in_array('system_id',array_keys($data)))
				$this->dsql('modify',false)->set('system_id',$this->api->getSystemId());
